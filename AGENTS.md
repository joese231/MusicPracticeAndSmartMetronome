...
src/
  ...
  lib/
    ...
    session/                  Pure session logic/runtime — blocks, driver state machine, tempo math, shared live-session hook, session artifact builders
...
**Persistence seam**: everything talks to `Repository` (src/lib/db/repository.ts), never HTTP directly. `FileRepository` proxies through Next.js API routes backed by JSON files in `data/`. Atomic write-then-rename + per-file mutex in `src/lib/db/fileStore.ts`. Read-modify-write API handlers must use `updateJsonAtomic(...)` so the read, mutation, and rename happen under the same per-file lock; plain `readJson()` + `writeJsonAtomic()` is only safe for full-file replacement with no dependency on the prior contents. JSON write APIs validate payloads before writing. Normal song/exercise edits use item-level PATCH routes with `expectedUpdatedAt` optimistic concurrency, delete routes, and reorder routes that only mutate `sortIndex`; normal PATCH payloads must not include `sortIndex`. Broad collection PUTs are validated legacy/bulk paths and are not used by normal Repository mutations. Statistics reset and factory reset go through dedicated server routes (`POST /api/statistics/reset`, `POST /api/factory-reset`) that mutate JSON files with server-side `updateJsonAtomic(...)`. New songs/exercises are inserted at the top of the manual order by assigning a `sortIndex` below the current minimum; drag/drop reorder normalizes visible order to `0..n`. Live song/exercise completion and item-backed manual logging use `Repository.completeSession()` / `POST /api/sessions/complete` so the session record append and latest item total increment happen in one server request, deduped by session id. Free Play and free-form manual records stay append-only through `Repository.appendSession()` / `POST /api/sessions`. `session-completion-ledger.json` records pending/applied completion totals so a retry can reconcile a session append whose item-total increment did not finish; clearing session history also clears this ledger. Session trim/delete rollbacks are coordinated server-side in `PATCH`/`DELETE /api/sessions/[id]`; other practice-time changes use `Repository.adjustPracticeTime()` / `PATCH /api/practice-time` so signed deltas apply to the latest persisted item row instead of replacing a stale client snapshot. `practice-time.json` is a best-effort song total mirror and must be cleared alongside session history during statistics resets/factory resets.
...
`SessionRecord.itemKind` is `"song" | "exercise" | "freePlay"`. Free-play and free-form manual records use `freePlay`; item-specific lists, last-played highlights, and song/exercise total adjustments should ignore `freePlay` records. The `/stats` manual logging details section stays collapsed by default directly below the stat summary cards. For item-specific manual logs, selecting a song/exercise seeds Starting Working BPM from the item's current `workingBpm`; Number of promotions creates working `PromotionEvent[]` rows and derives Ending Working BPM by repeatedly applying `step(...)` with that item's `stepPercent`.
...
Exercise block-template editing hides the `Trouble` tempo source and validation rejects trouble tempo/progression outside song Trouble Spot rows. Settings default block-template editors use local drafts with explicit Save/Revert actions; invalid intermediate drafts remain visible instead of being silently rejected.

**Recordings are NOT persisted.** Latest blob lives in `useSessionStore.latestRecording`, dies on next session start or reload, and is tagged with `{ itemKind, itemId, sessionId, plannedMinutes }` so future persistent recordings can disambiguate song, exercise, and free-play captures.
...
- `buildBlockPlan(minutes, song)` / `buildExerciseBlockPlan(exercise)` — discriminated block-plan entry points used by live pages. `ok: true` carries runnable blocks; `ok: false` carries `kind: "invalidTemplate" | "emptyBody"`, a reason, message, and any safe prefix blocks. Live pages do not auto-start when `ok` is false; they render a configuration error with an edit link instead.
- `buildBlocks(minutes, song)` — legacy song block-list helper that returns `buildBlockPlan(...).blocks`. Branches on `song.practiceMode`: `openEnded` returns one unbounded `openEnded` block; `timed` returns optional Conscious Practice plus one `timedPractice` countdown block, optionally no-metronome; `simple` returns optional Conscious Practice plus one `simpleMetronome` block at workingBpm; `smart` validates and allocates non-trouble blocks across the saved base duration, then expands additive `troubleSpot` recipes per spot. The session page reads `song.defaultSessionMinutes`; there is no one-off start duration query.
- `buildExerciseBlocks(exercise)` — legacy exercise block-list helper that returns `buildExerciseBlockPlan(...).blocks`. Branches in priority order: legacy `openEnded` or `practiceMode === "openEnded"` → single unbounded count-up; `simple` → optional Conscious + single steady-BPM block; `timed` → optional Conscious + single `timedPractice` countdown block; otherwise smart → optional Conscious + strictly validated recipe-allocated blocks. Exercise sessions read `exercise.sessionMinutes`.
...
Song and exercise live-session pages share `usePracticeSession` for metronome lifecycle, recorder lifecycle, RAF ticking, driver snapshot controls, keyboard shortcuts, pause/reset/previous/repeat/skip/end wiring, and toast state. `usePracticeSession` uses a start-generation token so async recorder/metronome setup from an unmounted or superseded start cannot later set live refs/state; unmount also cancels pending recorder streams. `SessionRecorder.stop()` resolves safely if the recorder is already inactive and rejects on recorder error events. Shared completion artifacts live in `lib/session/sessionArtifacts.ts`: song/exercise/free-play session-record builders, latest-recording construction, and intermission timing helpers. Page-specific code still owns item lookup, promotion semantics, BPM editing, and between-item routing overlays. Song/exercise pages settle queued BPM/promotion writes before finalizing a session record. The main live-session layout is rendered through `SessionShell`, which uses a wrapping toolbar and `min-h-dvh`/`overflow-x-hidden` root to keep controls usable on narrow screens. The inter-item overlay stacks controls vertically on narrow screens. Exercise intermission no longer exposes repeat-after-finalization; repeat remains an in-session block command only.
...
Song and exercise detail pages put the current mode/BPM summary and Start session controls before recording/history sections. Song detail hides ladder-derived tempo stats when a song has no working BPM, especially timed/open-ended no-metronome items. Free Play is globally discoverable from the home header as well as the exercise tab affordance.
...
npm run lint
npm run typecheck
npm run qa      # typecheck + tests + production build
...
- **Linting**: ESLint config (`.eslintrc.json`) uses the Next.js core-web-vitals preset; `npm run lint` calls the ESLint CLI with `--max-warnings=0`.
- **Testing**: Vitest suite covers 342 tests across 53 test files (tempo math, session state, duration allocation, tempo rules, manual logging, stats aggregates, form/list validation, template validation, metronome guards, segmented controls and React Testing Library interactions, file-backed persistence regressions, API validation, narrow item routes, store stale-write recovery, stats-page layout, exercise session completion wiring, shared session pause guards, settings/free-play accessibility checks, session-total reconciliation, reset/factory reset routes, live-session lifecycle guards, invalid block-plan runtime guards, detail-page/settings UI flow guards, session artifact helpers, latest-recording shape, and package QA scripts).
- **Build**: Next.js 15 production build tested and passing, all routes compile cleanly.
...
- **PWA**: do not add PWA/offline behavior until the Repository-level durable write queue in `docs/offline-write-queue-strategy.md` exists; queued commands need operation ids, reset barriers, replay/dedupe, and conflict handling before service-worker caching.

## Practice notes and assistant guidance

AGENTS.md is the canonical project guide for Codex and Claude; CLAUDE.md points here. Songs have optional nullable `notes` (legacy absence means empty); exercises retain their existing nullable notes. Forms and detail pages expose notes. Shared `components/session/PracticeNotes.tsx` displays and edits item notes on both live timer screens with explicit Save/Cancel and save-error feedback. Session shortcuts ignore focus inside the notes panel, including its buttons. Vitest discovers both `.test.ts` and `.test.tsx` files. Saves use the existing Repository-backed item stores and live-page pending-write tracking. Notes are item-level, not session-history snapshots. Exercise sessions and settings support 0.5–60 minutes in half-minute increments; smart templates still must exactly allocate their selected duration, so shorter sessions need matching block durations.

## Commit and push preference

The user explicitly wants personal practice data committed and pushed with app changes. For future commit/push requests, include all changed durable app data in `data/` by default, including songs, exercises, settings, session history, practice-time totals, and the session-completion ledger. Do not exclude these files as personal or unrelated data or ask for permission again. Check for data changes when staging. Temporary atomic-write `data/*.tmp` files are implementation artifacts, not durable app data, and remain ignored. Never overwrite or reset practice data to clean the working tree.
