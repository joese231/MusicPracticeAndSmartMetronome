...
- **Testing**: Vitest suite covers 321 tests across 52 test files (tempo math, session state, duration allocation, tempo rules, manual logging, stats aggregates, form/list validation, template validation, metronome guards, segmented controls and React Testing Library interactions, file-backed persistence regressions, API validation, narrow item routes, store stale-write recovery, stats-page layout, exercise session completion wiring, shared session pause guards, settings/free-play accessibility checks, session-total reconciliation, reset/factory reset routes, live-session lifecycle guards, invalid block-plan runtime guards, detail-page/settings UI flow guards, session artifact helpers, latest-recording shape, and package QA scripts).
