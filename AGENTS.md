...
`BlockTemplateEditor` uses wrapping control groups with explicit minimum widths for Duration, Tempo, Progression, and Metronome controls. Keep this layout responsive; fixed-minute decimal inputs such as `1.5` must not be obscured by adjacent tempo controls.
